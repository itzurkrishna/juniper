...
def assemble_smsdata(smsdata, args):
    ...
    smsdata.singleNssai.MergeFrom(snssai)
    mapEntry = smsdata.dnnConfigurations[args.dnn_name]
    mapEntry.pduSessionTypes.defaultSessionType.pduSessTypes = args.pdu_sess_types
    arrayEntry = mapEntry.pduSessionTypes.allowedSessionTypes.add()
    arrayEntry.pduSessTypes = args.allowed_sess_types
    mapEntry.internal_5gQosProfile.internal_5qi = args.qos_profile_5qi
    mapEntry.internal_5gQosProfile.arp.priorityLevel = args.priority_level
    mapEntry.sessionAmbr.uplink = args.dnn_ambr_ul
    mapEntry.sessionAmbr.downlink = args.dnn_ambr_dl
    mapEntry.sscModes.defaultSscMode.sscModes = args.default_ssc_mode
    arrayEntry = mapEntry.sscModes.allowedSscModes.add()
    arrayEntry.sscModes = args.allowed_ssc_modes
...
def add_subscriber(client, args):
    ...
    #added
    smsdata = pmn_subs_data.plmnSmData[args.mcc + "-" + args.mnc]
    assemble_smsdata(smsdata, args)
    # till here
...
def create_parser():
    ...
    for cmd in [parser_add]:
        ...
        #added
        parser_add.add_argument("--pdu_sess_types", help="PDU session types for DNN")
        parser_add.add_argument("--allowed_sess_types", help="Allowed session types for DNN")
        parser_add.add_argument("--qos_profile_5qi", help="Internal 5QI value for DNN")
        parser_add.add_argument("--priority_level", help="Priority level for DNN")
        parser_add.add_argument("--dnn_ambr_ul", help="Session AMBR uplink for DNN")
        parser_add.add_argument("--dnn_ambr_dl", help="Session AMBR downlink for DNN")
        parser_add.add_argument("--default_ssc_mode", help="Default SSC mode for DNN")
        parser_add.add_argument("--allowed_ssc_modes", help="Allowed SSC modes for DNN")
        #till here
